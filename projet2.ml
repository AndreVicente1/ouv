...
let compressionParListe g liste_deja_vus =
  let rec compress_node node =
    match node with
    | Leaf _ -> ()
    | Node (_, left, right) ->
      let leaf_list = liste_feuilles node in
      let half_length = List.length leaf_list / 2 in
      let second_half = drop half_length leaf_list in
      if List.for_all (fun x -> not x) second_half then
        right := !left
      else
        let n = [Int64.of_int (composition leaf_list)] in
        let found = ref false in
        List.iter (fun (int_list, ref_node) ->
          if int_list = n then (
            found := true;
            right := !ref_node;
            ref_node := !left
        ...
        if not !found then
          liste_deja_vus := (n, left) :: !liste_deja_vus;
      compress_node !left;
      compress_node !right
  in
  compress_node g
...
let com = compressionParListe decision_tree_25899 (ref []) ;;
